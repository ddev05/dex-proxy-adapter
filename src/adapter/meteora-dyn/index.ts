...
import {
  calculatePoolInfo,
  calculateProtocolTradingFee,
  calculateTradingFee,
  calculateWithdrawableAmount,
  computeOutAmount,
  createMeteoraSwapInstruction,
  getAmountByShare,
  getUnmintAmount,
  LP_MINT_PREFIX,
...
export class MeteoraDynAdapter implements IDexReadAdapter {
  ...
  static async create(
    ...
    const [dataA, dataB] = await connection.getMultipleAccountsInfo([
      poolInfo.aVault,
      poolInfo.bVault,
    ]);
  ...
  async getPoolReserves(): Promise<PoolReserves> {
    try {
      if (!this.poolInfo || !this.poolVaultAState || !this.poolVaultBState)
        return {
          //  @ts-ignore
      ...
      const [aVaultLpMint] =
        VAULT_WITH_NON_PDA_BASED_LP_MINT[this.poolInfo.aVault.toBase58()] ==
        undefined
          ? PublicKey.findProgramAddressSync(
              [Buffer.from(LP_MINT_PREFIX), this.poolInfo.aVault.toBuffer()],
              METEORA_VAULT_PROGRAM
            )
          : [VAULT_WITH_NON_PDA_BASED_LP_MINT[this.poolInfo.aVault.toBase58()]];
      const [bVaultLpMint] =
        VAULT_WITH_NON_PDA_BASED_LP_MINT[this.poolInfo.bVault.toBase58()] ==
        undefined
          ? PublicKey.findProgramAddressSync(
              [Buffer.from(LP_MINT_PREFIX), this.poolInfo.bVault.toBuffer()],
              METEORA_VAULT_PROGRAM
            )
          : [VAULT_WITH_NON_PDA_BASED_LP_MINT[this.poolInfo.bVault.toBase58()]];
      ...
      if (
        ...
        !bVaultData ||
        !lpMintData ||
        this.poolVaultAState == null ||
        this.poolVaultBState == null
      ) {
        return {
      ...
      this.poolVaultALp = BigNumber(
        AccountLayout.decode(baseVaultData.data).amount
      );
      this.poolVaultBLp = BigNumber(
        AccountLayout.decode(quoteVaultData.data).amount
      );
      this.vaultALpSupply = BigNumber(
        MintLayout.decode(aVaultLpMintData.data).supply
      );
      this.vaultBLpSupply = BigNumber(
        MintLayout.decode(bVaultLpMintData.data).supply
      );
      this.poolLpSupply = BigNumber(MintLayout.decode(lpMintData.data).supply);
      this.aVaultParsedData = BigNumber(
        AccountLayout.decode(aVaultData.data).amount
      );
      this.bVaultParsedData = BigNumber(
        AccountLayout.decode(bVaultData.data).amount
      );
  ...
  getSwapQuote(
    ...
    if (!this.poolInfo || !this.poolVaultAState || !this.poolVaultBState)
      throw new Error("Pool Info didn't be loaded");
    ...
    const sourceVaultWithdrawableAmount = calculateWithdrawableAmount(
      currentTime,
      swapSourceVault
    );
    ...
    const sourceVaultTotalAmount = sourceVaultWithdrawableAmount.plus(
      sourceAmountLessProtocolFee
    );
    ...
    const actualSourceAmount = afterSwapSourceAmount.minus(
      beforeSwapSourceAmount
    );
    let sourceAmountWithFee = actualSourceAmount.minus(
      tradeFeeAfterProtocolFee
    );
  ...
  getSwapInstruction(
    ...
    const { aVault, aVaultLp, bVault, bVaultLp, protocolTokenBFee } =
      this.poolInfo;
    const [aTokenVault] = PublicKey.findProgramAddressSync(
      [Buffer.from(TOKEN_VAULT_PREFIX), aVault.toBuffer()],
      METEORA_VAULT_PROGRAM
    );
    const [bTokenVault] = PublicKey.findProgramAddressSync(
      [Buffer.from(TOKEN_VAULT_PREFIX), bVault.toBuffer()],
      METEORA_VAULT_PROGRAM
    );

    const [aVaultLpMint] =
      VAULT_WITH_NON_PDA_BASED_LP_MINT[aVault.toBase58()] == undefined
        ? PublicKey.findProgramAddressSync(
            [Buffer.from(LP_MINT_PREFIX), aVault.toBuffer()],
            METEORA_VAULT_PROGRAM
          )
        : [VAULT_WITH_NON_PDA_BASED_LP_MINT[aVault.toBase58()]];
    const [bVaultLpMint] =
      VAULT_WITH_NON_PDA_BASED_LP_MINT[bVault.toBase58()] == undefined
        ? PublicKey.findProgramAddressSync(
            [Buffer.from(LP_MINT_PREFIX), bVault.toBuffer()],
            METEORA_VAULT_PROGRAM
          )
        : [VAULT_WITH_NON_PDA_BASED_LP_MINT[bVault.toBase58()]];

    const outputMint =
      this.poolInfo.tokenAMint == inputMint
